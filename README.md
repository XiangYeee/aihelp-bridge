...
```bash
# CLI
cursor-agent plugin marketplace add https://github.com/XiangYeee/aihelp-stack
```

或 Cursor IDE：Customize → Plugins → Add marketplace，仓库填 `https://github.com/XiangYeee/aihelp-stack`。导入后立刻打开插件 Configure，填写 5 项：AIHELP_MCP_HOST、AIHELP_MCP_BASIC、AIHELP_GITLAB_MCP_TOKEN、JUMPSERVER_MCP_ACCESSKEY、AIHELP_LANGFUSE_MCP_BASIC。值不要带 Basic / Bearer / AccessKey 前缀。
